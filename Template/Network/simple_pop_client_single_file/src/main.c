...
char *HELP = "	[ 0 ] Read && save NEW mail. \n"
             " 	[ 1 ] Read && save mail.\n"
			 "  [ 2 ] Send mail.\n      "
             "  [ 3 ] Delete NEW mail.\n"
			 "  [ 4 ] Delete OLD mail.\n"
...
int checkConn( char * inServer, int port )
{
	...
	char pop_server[1024] = { 0 };
	struct hostent *host_info = NULL;

	printf("Input pop server:");
	scanf("%s", pop_server);

	host_info = gethostbyname(pop_server);
	if (host_info == NULL) {
	...
	memcpy(&servaddr.sin_addr, *host_info->h_addr_list, sizeof(servaddr.sin_addr));
...
int sendMail( char * outServer, int port, char *user, char *pass )
{
    ...
    strcat( cmd, "MAIL FROM:2368877197@QQ.COM\x0d\x0a" ); // <<<<<<<<<<<<< ADD USER
...
int main()
{
    ...
    while ( 1 )
    {
        ...
        switch( option )
        {
            case 0:  readNewMail( sockfd );       
					 break;
            case 1:  readOldMail();               
					 break;
            case 2:  sendMail( outServer, SMTP, User, Pass ); 
					 break;
            case 3:  delFromServer( sockfd );     
					 break;
            // case 4:  delFromLocal( sockfd );      
            case 4:  delFromLocal();      
					 break;
