...
struct list_node{
	...
	struct Ordinary_passenger_node *Ordinary_passenger_list_head;
...
int msg_send(char *buf,int recv_id)
{
	...
	printf("NOT FOUNT USER!\n");
	pthread_mutex_unlock(&m);
	return -1;
}
*/

int msg_send(char *buf,int connfd)
{
...
//乘客信息大结构体
struct Ordinary_passenger_node
{
...
//初始化头节点
struct Ordinary_passenger_node *request_link_Ordinary_passenger_list_node(void)
{
    struct Ordinary_passenger_node *new_node;

    new_node = malloc(sizeof(struct Ordinary_passenger_node));
    if(new_node == NULL)
    {
        perror("申请内存失败");
        return NULL;
...
//尾插
void insert_Ordinary_passenger_list_node_tail(struct Ordinary_passenger_node *Ordinary_passenger_list_head, struct Ordinary_passenger_node *Ordinary_passenger_insert_node)
{  
...
//判断这个人是否已经注册过
struct Ordinary_passenger_node *find_usr(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *name)
{

	
	
	struct Ordinary_passenger_node *pos,*n;
	//printf("name1 = %s\n",name);
...
//判断这身份证是否已经注册过
struct Ordinary_passenger_node *find_usr_id(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *id)
{
	struct Ordinary_passenger_node *pos,*n;
...
//判断这电话号码是否已经注册过
struct Ordinary_passenger_node *find_usr_phone_number(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *phone_number)
{
	struct Ordinary_passenger_node *pos,*n;
...
//分割文件中中的数据
void strtok_fun(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *buf)
{
	//1. 为新节点申请空间。
	struct Ordinary_passenger_node *new;

    new = request_link_Ordinary_passenger_list_node();
    ...
    insert_Ordinary_passenger_list_node_tail(Ordinary_passenger_list_head,new);
    
	return;
}

//读文件
void read_file(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *filename)
{
	//1. 打开文件。
	...
	//4. 分割数据。
	strtok_fun(Ordinary_passenger_list_head,buf);
	
	return;
}

//初始化已注册（在文件夹中的）老用户
void init_old_usr(struct Ordinary_passenger_node *head)
{
	//1. 打开目录。
...
//显示单个用户信息
void printer_passenger_information(struct Ordinary_passenger_node *Ordinary_passenger_pos,int connfd)
{
...
//遍历所有用户信息
void show_all_login_usr(struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{
	struct Ordinary_passenger_node *pos;

    list_for_each_entry(pos,&(Ordinary_passenger_list_head-> Ordinary_passenger_list),  Ordinary_passenger_list)
    {
        printer_passenger_information(pos,connfd);
    }	
}

//显示余额
void printer_passenger_balance(struct Ordinary_passenger_node *Ordinary_passenger_pos,int connfd)
{
	bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"名字:%s\t",Ordinary_passenger_pos->info.name);
	send(connfd,sendbuf,strlen(sendbuf),0);
...
//注册时创建文件夹
int create_friend_dir(char *name)
{
	char dirname[30];
...
//注册时创建朋友圈文件夹
int create_Circle_of_friends_list_head_dir(char *name)
{
	char dirname[30];
...
//注册
int register_passage_information(struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{
    struct Ordinary_passenger_node *new_node;

    new_node = request_link_Ordinary_passenger_list_node();
	...
	if(find_usr(Ordinary_passenger_list_head,new_node->info.name) != NULL)
	{
		bzero(sendbuf,sizeof(sendbuf));
	...
	if(find_usr_id(Ordinary_passenger_list_head,new_node->info.id) != NULL)
	{
		bzero(sendbuf,sizeof(sendbuf));
	...
	while(1)
	{
		...
		if(find_usr_phone_number(Ordinary_passenger_list_head,new_node->info.phone_number) != NULL)
		{
			bzero(sendbuf,sizeof(sendbuf));
    ...
    insert_Ordinary_passenger_list_node_tail(Ordinary_passenger_list_head, new_node);
	
	create_friend_dir(new_node->info.name);
	
	create_Circle_of_friends_list_head_dir(new_node->info.name);
...
//登录初始化头节点
struct login_user *request_link_login_user_list_node(void)
{
...
//遍历所有登陆者信
int find_all_login_usr(struct login_user *login_user_list_head,char *name)
{
...
//登录尾插
void insert_login_user_list_node_tail(struct login_user *login_user_list_head, struct login_user *login_user_insert_node)
{  
    list_add_tail(&(login_user_insert_node->login_user_list), &(login_user_list_head->login_user_list));
}

//判断是否登录成功并将用户信息存进登录链表
int login_in(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd,struct login_user *login_user_list_head)
{
	
	struct login_user *new_node;

    new_node = request_link_login_user_list_node();
	...
	struct Ordinary_passenger_node *Ordinary_passenger_pos;
	
	Ordinary_passenger_pos = find_usr(Ordinary_passenger_list_head,name);
	...
	if(find_all_login_usr(login_user_list_head,name) == -1)
	{
		bzero(sendbuf,sizeof(sendbuf));
	...
	insert_login_user_list_node_tail(login_user_list_head,new_node);
...
//返回登录用户的信息
struct Ordinary_passenger_node *search_login_in_user(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head)
{
	struct Ordinary_passenger_node *pos;
...
//修改用户信息
int change_passage_information(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	
	struct Ordinary_passenger_node *Ordinary_passenger_pos;
	Ordinary_passenger_pos = search_login_in_user(Ordinary_passenger_list_head,login_in_list_head);//获取登陆者信息
	...
	char buf[50];
		...
		switch(a)
		{
			...
			printer_passenger_information(Ordinary_passenger_pos,connfd);
			break;
...
//重置密码
int reset_passwd(struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{
	char name[30];
	...
	pos = find_usr(Ordinary_passenger_list_head,name);
...
//初始化已删除（在文件夹中的）航班信息
void init_rm_flight_num_old_usr(struct flight_num_pas_node *rm_flight_num_list_head)
{
	//1. 打开目录。
...
//打印出一个航班信息
void printer_flight_num_information(struct flight_num_pas_node *flight_num_pos,int connfd)
{
	bzero(sendbuf,sizeof(sendbuf));
...
//显示所有航班信息
void show_all_flight_num(struct flight_num_pas_node *flight_num_list_head,int connfd)
{
	struct flight_num_pas_node *pos;

    list_for_each_entry(pos, &flight_num_list_head->flight_num, flight_num)
    {
        printer_flight_num_information(pos,connfd); 
    }
}

//通过目的地寻找航班
int pass_through_destination_find_info_list_node(struct flight_num_pas_node *flight_num_list_head,int connfd)
{  
...
//通过日期寻找航班信息
int pass_through_flight_date_find_info_list_node(struct flight_num_pas_node *flight_num_list_head,int connfd)
{  
	int count = 0;
    struct flight_num_pas_node *pos;
    char date[20];
...
//通过机型寻找航班信息
int pass_through_flight_model_find_info_list_node(struct flight_num_pas_node *flight_num_list_head,int connfd)
{  
	int count = 0;
    struct flight_num_pas_node *pos;
    char model[20];
...
//通过机票价格寻找航班信息
int pass_through_flight_price_find_info_list_node(struct flight_num_pas_node *flight_num_list_head,int connfd)
{  
...
//通过航班号寻找航班信息
int pass_through_flight_number_find_info_list_node(struct flight_num_pas_node *flight_num_list_head,int connfd)
{  
	int count = 0;
    struct flight_num_pas_node *pos;
    char flight_number[20];
...
//移动航班信息件
int cp_a_flight_num_file(struct flight_num_pas_node *flight_num_pos,int connfd)
{      
    char buf[30];
...
//删文航班信息件
int rm_a_flight_num_file(struct flight_num_pas_node *flight_num_pos,int connfd)
{      
    char buf[30];
    strcpy(buf,flight_num_pos->info.flight_number);
...
//把已删除航班放入链表
int put_rm_flight_num_information(struct flight_num_pas_node *rm_flight_num_list_head,struct flight_num_pas_node *flight_num_pos)
{
    struct flight_num_pas_node *new_node;
...
//删除航班信息
struct flight_num_pas_node *delete_flight_number(struct flight_num_pas_node *flight_num_list_head,char *flight_number,struct flight_num_pas_node *rm_flight_num_list_head)
{
...
//显示自己的实名认证
void find_check_identity_information(struct Verified_identity_pas_node *Verified_identity_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	struct Verified_identity_pas_node *pos;
...
//判断支付密码是否正确
int pay_password(struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	char pay_passwd[8];
	char buf[20];
	bzero(sendbuf,sizeof(sendbuf));
...
//查实名认证(购票)
int check_Verified(struct Ordinary_passenger_node *login_in_list_head,struct Verified_identity_pas_node *Verified_identity_list_head)
{
    struct Verified_identity_pas_node *pos;
    list_for_each_entry(pos,&(Verified_identity_list_head->Verified_list),Verified_list)
	{
...
//查实名认证特殊身份(购票)
int check_identity_info(struct Ordinary_passenger_node *login_in_list_head,
								struct Verified_identity_pas_node *Verified_identity_list_head)
{
    struct Verified_identity_pas_node *pos;
    list_for_each_entry(pos,&(Verified_identity_list_head->Verified_list),Verified_list)
...
//判断vip
int judging_the_identity(struct Ordinary_passenger_node *login_in_list_head)
{
...
//买票扣款
int Operational_balance  (struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head,int price,int connfd)
{
    ...
    //printf("price = %d",login_in->info.bal);
...
//买保险扣款
int Operational_balance_insurance  (struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head,int price)
{
...
//买票加积分
int Operational_integral(struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head)
{
...
//买票
int buy_ticket_info(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
													struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{   
...
//打印自己购买的机票
int show_user_ticket_buyer(struct Ordinary_passenger_node *login_in_list_head,struct buy_ticket_node *buy_ticket_list_head,int connfd)
{
	struct buy_ticket_node *pos;
    int count=0;
    list_for_each_entry(pos, &buy_ticket_list_head->pes_buy_ticket,pes_buy_ticket)
...
//返回登录用户的信息(退票)
struct Ordinary_passenger_node *Refund_search_login_in_user(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head)
{
	struct Ordinary_passenger_node *pos;
...
//查实名认证特殊身份(退票)
int Refund_check_identity_info(struct Ordinary_passenger_node *login_in_list_head,
								struct Verified_identity_pas_node *Verified_identity_list_head)
{
    struct Verified_identity_pas_node *pos;
    list_for_each_entry(pos,&(Verified_identity_list_head->Verified_list),Verified_list)
...
//寻找退票节点
struct buy_ticket_node *find_Refund_ticket(struct Ordinary_passenger_node *login_in_list_head,struct buy_ticket_node *buy_ticket_list_head,char *flight_number)
{
	struct buy_ticket_node *pos;
    int count=0;
    list_for_each_entry(pos, &buy_ticket_list_head->pes_buy_ticket,pes_buy_ticket)
...
//退票退款
int Operational_Refund_balance(struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head,int price)
{
...
//退票扣积分
int Refund_Operational_integral(struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head)
{
...
//退票
int Refund(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
													struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{   
...
//退票(改签)
int Change_Refund(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
													struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{   
...
//改签
int Change(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,
							struct Verified_identity_pas_node *Verified_identity_list_head,
									struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd)
{   
					...
					Verified_identity_list_head,Ordinary_passenger_list_head,connfd) == -1)
	{
		return -1;	
				...
				show_all_flight_num(flight_num_list_head,connfd);
...
//买保险扣款(帮别人买票)
int help_other_Operational_balance_insurance  (struct Ordinary_passenger_node *Ordinary_passenger_pos,int price)
{
...
//买票扣款(帮别人买票)
int help_other_Operational_balance  (struct Ordinary_passenger_node *Ordinary_passenger_pos,int price)
{
...
//买票加积分(帮别人买票)
int help_other_Operational_integral(struct Ordinary_passenger_node *Ordinary_passenger_pos)
{
...
//返回登录用户的信息(帮别人买票)
struct Ordinary_passenger_node *help_other_search_login_in_user(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *other_name)
{
	struct Ordinary_passenger_node *pos;
    list_for_each_entry(pos, &Ordinary_passenger_list_head->Ordinary_passenger_list,Ordinary_passenger_list)
    {    
...
//帮别人买票
int help_other_buy_ticket_info(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
													struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd,struct friends_info_list *friends_list_head)
{   
...
//退票退款(删航班)
int delete_flight_number_Operational_Refund_balance(struct Ordinary_passenger_node *Ordinary_passenger_pos,int price)
{
...
//退票扣积分(删航班)
int delete_flight_number_Refund_Operational_integral(struct Ordinary_passenger_node *Ordinary_passenger_pos)
{
...
//返回登录用户的信息(删航班退票)
struct Ordinary_passenger_node *delete_flight_number_Refund_search_login_in_user(struct Ordinary_passenger_node *Ordinary_passenger_list_head,char *name)
{
	struct Ordinary_passenger_node *pos,*n;
...
int delete_flight_number_Refund(struct flight_num_pas_node *flight_num_pos,
										struct buy_ticket_node *buy_ticket_pos,struct Ordinary_passenger_node *Ordinary_passenger_list_head,
...
//删除航班并退票
int delete_flight_number_Refund_all(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *Ordinary_passenger_list_head,
									struct buy_ticket_node *buy_ticket_list_head,struct flight_num_pas_node *rm_flight_num_list_head,int connfd)
{
	...
	flight_num_pos = delete_flight_number(flight_num_list_head,flight_number,rm_flight_num_list_head);
	...
	cp_a_flight_num_file(flight_num_pos,connfd);
	rm_a_flight_num_file(flight_num_pos,connfd);
	put_rm_flight_num_information(rm_flight_num_list_head,flight_num_pos);
...
//开发票
int invoice(struct buy_ticket_node *buy_ticket_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	struct buy_ticket_node *buy_ticket_pos = NULL;
	char name[20];
	char flight_number[20];
...
//返回登录用户的信息(充值)
struct Ordinary_passenger_node *Recharge_search_login_in_user(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head)
{
	struct Ordinary_passenger_node *pos;
...
//充值操作
int Operational_Recharge_balance(struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head,int balance,int connfd)
{
    ...
    //printf("balance = %d",login_in->info.bal);
...
//充值 vip
int Recharge_Operational_vip(struct Ordinary_passenger_node *Ordinary_passenger_pos,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
...
//充值
int Recharge(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	int balance;
	char buf[100];
	bzero(sendbuf,sizeof(sendbuf));
...
void printer_friends_information(struct friends_info_list *friends_list_pos,int connfd)
{
	...
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"黑名单:%d\n",friends_list_pos->blacklist);
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
   	sprintf(sendbuf,"-------------------------------------------------------------------------\n");
	send(connfd,sendbuf,strlen(sendbuf),0);
...
//加好友
int add_friends(struct friends_info_list *friends_list_head,struct login_user *login_user_list_head,struct Ordinary_passenger_node *Ordinary_passenger_list_head,
						int connfd,char *user_name)
{
	...
	if(find_usr(Ordinary_passenger_list_head,friends_name) == NULL)
	{
		bzero(sendbuf,sizeof(sendbuf));
...
//给好友(发信息)
int send_msg_to_friends(struct friends_info_list *friends_list_head,struct login_user *login_user_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	char friends_name[50];
	struct login_user *login_user_pos;
	char buf[50];
	...
	if(find_friends_blacklist(friends_list_head,friends_name,connfd) != NULL)
	{
		bzero(sendbuf,sizeof(sendbuf));
		sprintf(sendbuf,"%s为黑名单\n",friends_name);
		send(connfd,sendbuf,strlen(sendbuf),0);
		return -1;
	}
	...
	while(1)
	{
		...
		msg_send(buf1,login_user_pos->connfd);
...
//初始化已保存的朋友圈列表
void init_old_Circle_of_friends(struct Circle_of_friends *Circle_of_friends_list_head,struct Ordinary_passenger_node *login_in_list_head,
...
//写朋友圈
int pyq(struct Circle_of_friends *Circle_of_friends_list_head,struct Ordinary_passenger_node *login_in_list_head,int connfd)
{
	struct Circle_of_friends *new_node;
...
int put_in_blacklist(struct friends_info_list *friends_list_head,char *user_name,int connfd)
{
	struct friends_info_list *friends_pos = NULL;
	friends_pos = malloc(sizeof(struct friends_info_list));
	char buf[50];
	char friends_name[50];
	bzero(sendbuf,sizeof(sendbuf));
   	sprintf(sendbuf,"请输入要拉入黑名单的名字\n");
	send(connfd,sendbuf,strlen(sendbuf),0);
	...
	friends_pos = find_friends(friends_list_head,friends_name,connfd);
    ...
    char filedata[2000]={0};
    sprintf(filedata,"%s,%d",friends_name,friends_pos->blacklist);
    ...
    char filedata1[2000]={0};
    sprintf(filedata1,"%s,%d",user_name,friends_pos->blacklist);

    FILE *fp = fopen(filename,"w+");
	FILE *fp1 = fopen(filename1,"w+");
	
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"黑名单写入成功\n");
	send(connfd,sendbuf,strlen(sendbuf),0);
...
int put_out_blacklist(struct friends_info_list *friends_list_head,char *user_name,int connfd)
{
	struct friends_info_list *friends_pos = NULL;
	friends_pos = malloc(sizeof(struct friends_info_list));
	char buf[50];
...
void friends_menu(int connfd)
{
	...
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"                   5. 发朋友圈                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"                   6. 刷自己朋友圈                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"                   7. 刷好友朋友圈                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"                   8. 刷新好友朋友圈                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	sprintf(sendbuf,"                   9. 黑名单                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
	sprintf(sendbuf,"                   10. 移除黑名单                      \n"); 
	send(connfd,sendbuf,strlen(sendbuf),0);
	bzero(sendbuf,sizeof(sendbuf));
...
//聊天界面switch
int friends_switch(struct friends_info_list *friends_list_head,int connfd,struct login_user *login_user_list_head,struct Ordinary_passenger_node *login_in_list_head,
								struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Circle_of_friends *Circle_of_friends_list_head,struct Circle_of_friends *Circle_of_friends_others_list_head)
{
	...
	while(1)
	{
		...
		switch(a)
		{
			...
			break;
			case 5:
			pyq(Circle_of_friends_list_head,login_in_list_head,connfd);
			show_my_Circle_of_friends(Circle_of_friends_list_head,connfd);
			break;
			case 6:
			show_my_Circle_of_friends(Circle_of_friends_list_head,connfd);
			break;
			case 7:
			print_Circle_of_friends_others(Circle_of_friends_others_list_head,connfd);
			break;
			case 8:
			free_Circle_of_friends_others(Circle_of_friends_others_list_head);
			init_all_friends_Circle_of_friends(Circle_of_friends_others_list_head,friends_list_head,connfd);
			break;
			case 9:
			put_in_blacklist(friends_list_head,login_in_list_head->info.name,connfd);
			break;
			case 10:
			put_out_blacklist(friends_list_head,login_in_list_head->info.name,connfd);
			break;
			case 0:
			goto flight_num_end;
			break;
...
//管理员登录后界面switch
int admin_switch(struct Ordinary_passenger_node *Ordinary_passenger_list_head,struct Ordinary_passenger_node *login_in_list_head,
									struct flight_num_pas_node *flight_num_list_head,
									struct Verified_identity_pas_node *Verified_identity_list_head,
									...
									struct flight_num_pas_node *rm_flight_num_list_head,int connfd)
{
	...
	while(1)
	{
		...
		switch(a)
		{
			...
			break;
			case 3:
			show_all_flight_num(rm_flight_num_list_head,connfd);
			break;
			/*case 4:
			...
			break;
			case 10:
			show_all_login_usr(Ordinary_passenger_list_head);
...
//买票switch
int buy_ticket(struct flight_num_pas_node *flight_num_list_head,struct Ordinary_passenger_node *login_in_list_head,
							struct buy_ticket_node *buy_ticket_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
													struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd,struct friends_info_list *friends_list_head)
{
	int a;
...
int check_flight_num_switch(struct flight_num_pas_node *flight_num_list_head,int connfd)
{
	...
	while(1)
	{
		...
		{
			case 1:
			if(pass_through_destination_find_info_list_node(flight_num_list_head,connfd) == 0)//通过目的地寻找航班信息
			{
				bzero(sendbuf,sizeof(sendbuf));
			...
			break;
			case 2:																					
			if(pass_through_flight_date_find_info_list_node(flight_num_list_head,connfd) == 0)//通过日期寻找航班信息
			{
				bzero(sendbuf,sizeof(sendbuf));
				sprintf(sendbuf,"无匹配航班\n");
				send(connfd,sendbuf,strlen(sendbuf),0);
			...
			break;
			case 3:
			if(pass_through_flight_model_find_info_list_node(flight_num_list_head,connfd) == 0)//通过机型寻找航班信息
			{
				bzero(sendbuf,sizeof(sendbuf));
				sprintf(sendbuf,"无匹配航班\n");
				send(connfd,sendbuf,strlen(sendbuf),0);
			...
			break;
			case 4:
			if(pass_through_flight_price_find_info_list_node(flight_num_list_head,connfd) == 0)//通过机票价格寻找航班信息
			{
				bzero(sendbuf,sizeof(sendbuf));
				sprintf(sendbuf,"无匹配航班\n");
				send(connfd,sendbuf,strlen(sendbuf),0);
			...
			break;
			case 5:
			if(pass_through_flight_number_find_info_list_node(flight_num_list_head,connfd) == 0)//通过航班号寻找航班信息
			{
				bzero(sendbuf,sizeof(sendbuf));
				sprintf(sendbuf,"无匹配航班\n");
				send(connfd,sendbuf,strlen(sendbuf),0);
...
int flight_num_switch(struct flight_num_pas_node *flight_num_list_head,struct Verified_identity_pas_node *Verified_identity_list_head,
							struct Ordinary_passenger_node *login_in_list_head,struct buy_ticket_node *buy_ticket_list_head,
											struct Ordinary_passenger_node *Ordinary_passenger_list_head,int connfd,struct login_user *login_user_list_head)
{
	...
	while(1)
	{
		...
		switch(a)
		{
			...
			break;
			case 5:
			printer_passenger_balance(login_in_list_head,connfd);
			break;
			case 6:
			if(pay_password(login_in_list_head,connfd) == -1)
			{
			...
			break;
			case 7:
			printer_passenger_information(login_in_list_head,connfd);
			break;
			case 8:
			change_passage_information(Ordinary_passenger_list_head,login_in_list_head,connfd);
			break;
			case 9:
...
void *routine(void *arg)
{
	...
	struct Ordinary_passenger_node *login_in_list_head  = NULL;
	login_in_list_head = request_link_Ordinary_passenger_list_node();
	
	struct Ordinary_passenger_node *Ordinary_passenger_list_head  = NULL;
	...
	while(1)
	{
			...
			{
				case 1:
				register_passage_information(Ordinary_passenger_list_head,peer->connfd);
				break;
				case 2:
				if(login_in(Ordinary_passenger_list_head,login_in_list_head,peer->connfd,login_user_list_head) == 0)
				{
...
int main(int argc,char *argv[])  //  ./server 50001
{
	...
	//乘客链表创建、初始化
	struct Ordinary_passenger_node *Ordinary_passenger_list_head  = NULL;
	Ordinary_passenger_list_head = request_link_Ordinary_passenger_list_node();
	
	init_old_usr(Ordinary_passenger_list_head);
...
//------------------------------------------------------------------------------
	...
	init_rm_flight_num_old_usr(rm_flight_num_list_head);

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
	
	struct login_user *login_user_list_head  = NULL;
	login_user_list_head =request_link_login_user_list_node();
