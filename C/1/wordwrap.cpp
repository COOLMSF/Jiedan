...
uint16_t consoleWidth;  //控制台宽度
uint16_t currentConsoleOffset;  //控制台偏移量

void initWordWrap() {    //初始化
...
void wrapOut(const std::string *text) {
    ...
    ostream_iterator<char> iout(cout); // Iterators for the string and display
    string::const_iterator strInt = text->begin();
    ...
    while (position < len) {
        ...
        if ((len - position) < left) { // Can we fit the whole string on this line?
            cout << &rawString[position] << " "; // Yes, just do it.
