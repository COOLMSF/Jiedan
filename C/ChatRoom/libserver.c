...
void *thread_client(void *ptr)
{
    ...
    while(1) {
        ...
        if (recv(user->sockfd, recv_buf, sizeof(recv_buf), 0) > 0){
			 ...
			 * check send command
			 */
			if (strstr(recv_buf, "send signature") != NULL) {

				/*
				 * multiple file support?
				 */
				filename = strtok(recv_buf, " ");
				...
				if (save_file(filename, user->sockfd) != -1) {
					printf("Saving file OK\n");
					continue;
				}

				printf("Saving file ERROR\n");
				continue;
			}
