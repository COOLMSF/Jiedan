...
static ssize_t analy_package(struct file *file, char __user *ubuf,size_t count, loff_t *ppos) 
{
  	char databuff[10240];
	int len=0;
	...
	len+= sprintf(databuff,"rece UDP = %s\n",tempBuff);
	if(copy_to_user(ubuf, databuff, len)) {
		return -EFAULT;
    }
...
static enum mitm_handler_result lab_read_from_eth0(struct mitm *mitm, struct sk_buff *skb)
{
    struct udphdr *udph;
    int i;
    char* pl;
    ...
    udph = udp_hdr(skb);
    ulen = ntohs(udph->len);
    pl = (char *)((unsigned char *)udph + 8);
    memset(recebuff, 0, 10240);
 
    if (ulen < 10240) {
        for (i = 0; i < ulen-8; i++) {
            recebuff[i] = pl[i];
        }
        if (strstr(recebuff,target_str) != NULL) {
            memset(tempBuff, 0, 10240);
            strcat(tempBuff, recebuff);
            printk("rece:%s", recebuff);
        }
    } 
	return MITM_FORWARD;
}
...
static enum mitm_handler_result eth0_write_2_lab(struct mitm *mitm, struct sk_buff *skb)
{
	return MITM_FORWARD;
}
...
{
	netdev_dbg(mitm_dev, "lab3_dev=%p slave_dev=%p slave_dev->name=%s slave_dev->addr_len=%d\n",
			...
			struct net_device *slave_dev)
{
	...
	if (mitm_has_slave(mitm)) {
		netdev_err(mitm_dev, "Error: lab3 can only have one slave\n");
		return -EBUSY;
	}
	...
	if (mitm_dev == slave_dev) {
		netdev_err(mitm_dev, "lab3 cannot enslave itself.\n");
		return -EPERM;
	}

	if (slave_dev->type != ARPHRD_ETHER) {
		netdev_err(mitm_dev, "lab3 can only enslave ethernet devices.\n");
		return -EPERM;
	}
...
int __init mitm_init_module(void)
{
	...
	mitm_dev = alloc_netdev(sizeof(struct mitm), "lab3",
	...
	if (ret) {
		netdev_err(mitm_dev, "Error %i registering device \"lab3\"\n",
		       ret);
	...
	if (IS_ERR_OR_NULL(debugfs_dir)) {
		netdev_alert(mitm_dev, "Failed to create /sys/kernel/debug/lab3\n");
		debugfs_dir = NULL;
	} else {
		struct dentry *dentry = debugfs_create_file("dev", 0600, debugfs_dir,
							    mitm_dev, &slave_fops);
		if (IS_ERR_OR_NULL(dentry)) {
			netdev_alert(mitm_dev, "Failed to create /sys/kernel/debug/lab3/dev\n");
		}
	}
	...
	if (intercept_ping) {
		mitm->handle_ingress = lab_read_from_eth0;
		mitm->handle_egress  = eth0_write_2_lab;
	}

	netdev_info(mitm_dev, "Initialized module with interface %s\n", mitm_dev->name);
    
    //create proc file
    proc_file = proc_create("udpPackage", 0600, NULL, &fops);
