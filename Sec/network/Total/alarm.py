...
def engine():
    parser = argparse.ArgumentParser(description='A network sniffer')
    ...
    if args.pcapfile:
        try:
            print("Reading PCAPs file %(filename)s" % {"filename": args.pcapfile})
            sniff(offline=args.pcapfile, prn=packet_callback)
        except IOError:
            print("Something went wrong with the PCAP file %(filename)s IO!" % {"filename": args.pcapfile})
        ...
        try:
            ...
            print("Error opening network interface %(interface)s. It does not exist." % {"interface": args.interface})
        except Exception as e:
            print(type(e))
            print("Can\'t read network traffic. Are you root?")
